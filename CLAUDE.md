...
> Codex 只自動載入 `AGENTS.md`）。**改任何一份就要同步改另一份。**
> 唯一該有的差異是「工具專屬」那一節。
>
> 產生器會比對兩份最後被改的 commit，不一致就報 `agent-doc-drift`。
>
> 2026-08-30 之前這份只是一份指向 `AGENTS.md` 的指路檔 —— 也就是說 Claude Code
...
**Codex**（讀 `AGENTS.md`）：沒有額外規則，往下讀就好。
...
- **可寫**：只有 `<資料根目錄>` **頂層的 `.md`**（上線 runbook 那類文件）。帳本、備份與其他子資料夾一律不寫 —— 那些只由 App 自己寫。
- **不可讀、不可寫**：`<私人資料樹>\` 底下**其他任何**資料夾。
- **即使在允許範圍內，也不要把實際內容貼進對話。** 分兩級：
  - **摘要式比對可以直接說** —— 欄位名、筆數、日期範圍、「幾處不符」。這種描述已經足夠決定要不要動手、動哪裡。
  - **逐值列出要先問** —— 實際的帳戶名、金額、日期、第三方服務名，**即使那個值在某份文件裡已經有**（那份文件在資料側，對話不在）。

  判準是「**離開這段對話之後，這句話還能不能指認出一個人的什麼**」。寫進資料側檔案的不受此限 —— 這條限制的是對話，不是工作成果。

  > 這條 2026-09-05 破過一次：任務是拿 runbook 跟帳本對帳，而**對帳不指名就沒有結果**，
  > 於是帳戶名、本金與第三方服務名全進了對話。當時的規則只說「用筆數這種描述」，
  > 沒說對帳怎麼辦。**一條做不到的規則不會被遵守，只會被繞過**，所以改成兩級。

### `.claude\settings.json` 怎麼設、以及它的三個極限

`deny` 優先於 `allow`，而且路徑 pattern **沒有否定語法**，所以做不到「deny 整個 `<私人資料樹>\**` 但 allow 其中一個子樹」—— 那樣寫會連允許的資料夾一起擋掉。

> **2026-09-05 之前這裡是「寫入絕對禁止，無例外」，而那是錯的方向。** 資料根裡除了帳本，
> 還有上線 runbook 這類**本來就該被修改的文件**；一律 deny 的話它們改不動，合法的修改
> 只剩下走 shell —— 而下面第一個極限正是「攔不住 shell」。
> **閘門把守規矩的那條路擋死、只留沒有守門的那條，方向是反的。**

改成整棵樹的讀寫都 `ask`，硬擋改用**「只 deny 該擋的葉子」**—— 那個方向 glob 寫得出來：

| 規則 | 範圍 | 作用 |
|---|---|---|
| `ask` | `Read`／`Glob`／`Grep`／`Edit`／`Write` on `<私人資料樹>/**` | 一律先問過使用者 |
| `deny` | `Edit`／`Write` on 帳本與備份資料夾、以及 `*.sqlite`／`*.db` | **無例外。** 那些只由 App 自己寫，agent 永遠不需要寫 |
| `allow` | 讀三項 on `<資料根目錄>/**`；`Edit`／`Write` **只到** `<資料根目錄>/*.md` | 指定資料夾免問，寫入只放行頂層文件 |

`allow` 的 `*.md` 用**單一 `*`** 是刻意的 —— 它只 match 一層，底下的資料夾一個都不放行。
`deny` 則**一定要用 `/**` 結尾**，理由正好相反：單一 `*` 會涵蓋 `X` 這個資料夾本身卻涵蓋
不到 `X\secret.txt`。同一個 glob 特性，兩個方向各用一次。

> **這一份不是唯一一層。** `~\.claude\settings.json` 是「你，每個專案」，不管從哪個目錄
> 啟動都生效，而 `permissions` 這種清單鍵跨層是**合併**不是覆蓋。所以整棵樹的 `ask` 與
> 不綁專案名的葉子 `deny` 應該放在那裡，這一份只留這個專案自己的規則。
> **但專案層只在「從專案目錄啟動」時載入**，從上層工作區啟動時它完全不生效 ——
> 所以真正要硬擋的東西不能只寫在這一份。沒有使用者層基線的人（例如剛 clone 的人）
> 把兩邊合起來放進這一份即可，`settings.example.json` 給的就是合起來的完整版。

**三個極限，不要當成滴水不漏：**

1. **攔不住 shell。** `Get-Content`、`cat`、`type`、`Select-String` 都能讀檔，任意 shell 指令無法可靠地用 pattern 比對。用 shell 時靠的是這一節的成文規則。
2. **不能假設什麼時候生效。** 這裡原本寫「改了一定要重開 session 才會載入」，2026-09-05 實測不是：改完基線那兩層之後，同一個 session 裡原本被擋掉的 Edit **當場就寫得動了**。兩個方向都別假設 —— 要驗就用**全新 session** 驗，因為要驗的本來就是「全新 session 看到什麼」。
