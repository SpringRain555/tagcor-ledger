...
- **帳戶、類別、項目、模板都可以排成自己想要的順序**：按那一頁的「排序…」開一個
  視窗，用拖曳（或上移／下移）排好，按「確定」才存下來。常用的排前面。
  - 這份順序也用在**記帳頁的下拉選單**，所以每天記帳會少捲一點；
    帳戶的順序另外也決定**資產總覽**的列法。
  - **項目的視窗是兩欄**：左邊排類別、右邊排所選類別底下的項目。
  - 已封存的也會列出來（標「已封存」），這樣以後恢復它時位置才不會亂掉。
