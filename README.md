...
- 開啟即見的「資產總覽」：總資產、各帳戶餘額、下一筆定存到期、待辦。
...
- **帳戶、類別、項目、模板的排序都在那一頁的「排序…」視窗裡設**，而且**會記住**，
  下次開程式還是那個排法。視窗分兩半：
...
刪除只適用完全未被交易、盤點、模板、定期收支或待確認項目引用的設定項。已有歷史資料者請用封存，以維持帳務一致性。預設帳戶不可刪除，需先切換預設帳戶。

### 待確認

- 定期收支與定存到期只產生待確認項目，**不會自動入帳**。
