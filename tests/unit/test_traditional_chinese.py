...
2026-08-18 加入時對全專案實跑過，共移除五個誤報：`量`、`常`、`伙`、`抽`、`骨`，
它們在繁簡是同一個碼位。**日後要加字進來，先對全專案跑一次確認沒有新誤報** ——
一個會對正常寫作報錯的守門，比沒有守門更糟，因為你會學會忽略它。
"""
...
SIMPLIFIED_ONLY = set(
    ...
    "编码译释标准级联统划组织优势复杂难参传递归环继承属调返异错试盖"
    ...
    "殖毁气汇汉沟没泞泪浅浆浇济浏浑浓涛润涨渐温湾湿满滤滥灭灯灵灾炉点炼烟烦"
    "烧热爱爷牵状犹独狮献玛环玺现瑶璃电画畅疗疯痴皱盏盐监盗盘着睁矫矿码砖础"
