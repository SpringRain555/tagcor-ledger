...
2026-08-18 第三次：移除三個**兩用字**。它們是正當的繁體字，只是在某些詞裡同時也是
另一個字的簡化形：
...
`佣` 是掃到全國法規資料庫抓下來的**所得稅法第 88 條原文**才發現的 —— 官方法律條文
用了這個字，沒有比這更硬的「它是正當繁體字」的證據。

字表只收「簡體專用」字，兩用的不收。代價是「水准」「规划」這種寫法的後半個字抓不到
（前半個字仍然抓得到），但那比每次寫到「不准」「划算」都紅一次划算。
...
# 這些路徑放的是**抓下來的原始資料與由它產生的東西**，不是我們寫的字。
#
# 引用的法條原文必須逐字照抄 —— 就算它用了我們字表裡的字，也不該去「修正」別人的
# 法律條文。手寫的部分（reference/sources.json、reference/summaries.json）不在這裡，
# 仍然會被檢查。
SKIP_PATHS = {
    ("reference", "raw"),
    ("reference", "corpus"),
...
# **這裡只能放「簡體才有」的字。** 兩邊寫法相同的字進來就是誤判，而誤判的代價是
# 有人為了讓測試變綠去改一句本來就正確的中文。
#
# 已知被誤收過的：**返**（返家、返回、往返都是正體標準用字，2026-08-22 移除）。
# 這張表沒有逐字稽核過，再發現誤判就照樣移除並在這裡記一筆。
SIMPLIFIED_ONLY = set(
    "个们为这说关边现时从会单实义检测计认证记录数据库备简体转换类项资设误处"
    "态运长维护读权规则图网页显输选择确删执开启动结载储术语该问题决议论觉变"
    "编码译释标级联统组织优势复杂难参传递归环继属调异错试盖"
    "断与并无万专业东丝丢严丰临举乐习乡书买乱争亏云亚产亲亿仅仑仓仪价众伟"
    "伤伦伪余侠侣侦侧俭债倾偿儿兑兰兴兹养兽内冈册军农冯冲况冻净凉减凑几凤"
...
def _scanned_files() -> list[Path]:
    ...
    for path in sorted(PROJECT_ROOT.rglob("*")):
        ...
        if any(part in SKIP_DIRS for part in relative.parts):
            continue
        if any(relative.parts[: len(prefix)] == prefix for prefix in SKIP_PATHS):
            continue
