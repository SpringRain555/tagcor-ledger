...
> **先掃標題，不要從頭讀。** 每一條的 `##` 標題本身就是那一條的結論，
> `grep "^## " docs/lessons.md` 四十行看完，再挑相關的那幾條讀內文。
> 整份約 22k token，逐字讀完的成本遠高於它的價值。
>
> **2026-08-22 決定不分卷。** 分卷只會多一層「這件事在哪一卷」的查找，
> 而標題清單已經是索引了。真的該分卷的訊號不是行數，是**標題本身多到掃不完**
> （超過一百條左右）。
...
## 2026-08-23 移除一個功能時，最容易漏掉的是「它順便提供的那個入口」

**情境**：移除定期收支（[ADR-0011](decisions/ADR-0011-drop-recurring-schedules.md)）。

**做了什麼**：把兩張表、兩個 store、一個分頁、一個 service 的一半全部拿掉，
測試也照著改。

**為什麼失敗**：定存的待確認項目**沒有手動觸發的入口了**。以前
`controller.generate_due()` 一次做兩件事（排程 ＋ 定存），而唯一按得到它的按鈕
在定期收支頁上。那一頁走了，定存就只剩「開程式時產生一次」。

症狀不是錯誤 —— 是「程式開著的時候剛建了一份合約，待確認頁永遠是空的」，
而使用者只會覺得功能壞了。發現它純粹是因為改測試時 `generate_due()` 不見了。

**結論**：定存頁補上「產生到期與領息項目」。**README 從以前就寫著這顆按鈕存在**，
程式裡卻沒有 —— 那份文件比程式更早知道它該在哪裡。

**不要再做**：

- 移除功能之前，先問「**有沒有別的東西是透過它才碰得到的**」。共用的 method、
  共用的按鈕、共用的頁面都算。`grep` 那個 method 的每一個呼叫端，逐一問
  「它拿掉之後，這件事還做得到嗎」。
- **測試改不動的地方就是那個訊號。** 測試裡呼叫不到某個東西了，先想「使用者是不是
  也呼叫不到了」，不要直接換一個內部 API 繞過去。

---

## 2026-08-23 兩份重複的 formatter，只有沒人用的那一份是錯的

**情境**：待確認只剩定存一個來源之後，`inbox_values()` 與 `deposit_event_values()`
變得一字不差。

**做了什麼**：留 `deposit_event_values()`，把待確認頁指過去。

**為什麼失敗**：`deposit_event_values()` 的日期是 `str(item["due_date"])`，
印的是 ISO 字串；`inbox_values()` 是 `display_date()`。**兩份看起來一樣，其實不一樣**
—— 而錯的那一份剛好是沒有任何頁面用它、所以從來沒有人看過的那一份。
差一點就把畫面上的 `2021/01/15` 換成 `2021-01-15`。

抓到它的是一條在斷言 `"/" in date_text` 的測試，不是我讀程式讀出來的。

**結論**：`deposit_event_values()` 改用 `display_date()`。

**不要再做**：合併兩份「看起來一樣」的實作時，**不要用眼睛比對**。
- **先確認哪一份是真的在畫面上跑的**，保留那一份的行為。
- 沒有呼叫端的那一份等於沒有被驗證過，它的細節不能當成正確答案。
- 合併之後跑一次會斷言**輸出字串**的測試 —— 型別對得上不代表格式對得上。

---

## 2026-08-23 深色主題把「選取」的明度預算用完了，卻沒有換一個維度

**情境**：使用者回報「操作設定裡選取欄位中的項目時，選取的顯示效果不明確」。

**做了什麼**：選取列一直只靠底色 —— `SELECTED #26262E` 對一般列底 `SURFACE #16161A`。

**為什麼失敗**：那組對比是 **1.20**。人眼在 1.2 幾乎分不出來，所以「不明確」不是感覺，
是量得出來的。而且**調亮解決不了**：亮到 `#333340` 時支出紅對選取列的對比就掉到 4.36，
破掉「所有文字／底色組合 >= 4.5」那條守門 —— 而選取列正是使用者最想看清楚金額的時候。
上限是 `#2E2E38`，只有 1.34。

根因是**在一個維度上同時放兩個需求**：中性純灰的深色主題只有明度可用，而明度已經
被「金額要讀得到」佔滿了。

**結論**：底色亮到上限，再加上下兩條 1px 橫線。**線是形狀，不吃明度預算。**

**不要再做**：

- 深色主題裡不要用「比周圍亮一點」當成唯一的狀態線索。**先算對比**，1.2 這種數字
  代表這條路走不通，不是「再亮一點就好」。
- **加框線一定要同步收 `padding`。** Qt 的 `::item` 是 content-box，而列高被
  `defaultSectionSize()` 釘死 —— 直接加 2px 框線就是從中文的可用高度裡扣，
  症狀是「選取的那一列字看起來怪怪的」，而不會有任何東西變紅。
  7px 配 0 框線 ＝ 6px 配 1px 框線，現在有測試逐項對算。

---

## 2026-08-23 「排序…」的刪節號被讀成「文字被截斷了」

**情境**：使用者問「排序按鈕顯示是預設『排序...』嗎？還是被吃掉顯示文字了？」

**做了什麼**：按鈕文字是 `排序…`，用 U+2026 —— 桌面軟體的慣例，代表「按下去會開一個
視窗問你」。

**為什麼失敗**：實際量過**沒有任何東西被截斷**（3 個字元、48px，按鈕遠比它寬）。
但兩個中文字後面接一個刪節號，看起來就跟被 `elidedText()` 切掉的字一模一樣。
慣例要有效，前提是讀的人認得那個慣例 —— 而這個專案的使用者只有一位，他不認得。

**結論**：改成「排序設定」。**沒有 bug，但那個慣例在這裡沒有發揮作用，就等於沒用。**

**不要再做**：不要用「這是標準慣例」當成不改的理由。慣例的價值在於被認出來；
唯一的使用者說他讀成別的意思，那就是讀成別的意思。
短標籤配符號時，優先讓**用詞本身**說得出那顆按鈕做什麼。

---

## 2026-08-23 文件用權威的語氣描述了一件程式沒做的事，而沒有守門看得出來

**情境**：使用者問「操作設定裡模板的『封存』跟刪除有什麼不一樣？」

**做了什麼**：查 `state-machines.md` §5，它寫著模板與定期收支
「轉移與帳戶／類別相同（`active ↔ archived`）」。

**為什麼失敗**：那句話從 v0.7 起就是假的。`restore_template` / `restore_schedule`
在 store、application、controller **三層都不存在**，而兩個分頁的 `refresh()` 也沒有帶
`include_archived` —— 所以「封存」實際上是單向的，按下去那一列就從畫面上永遠消失。

沒有任何東西會發現：`test_docs_drift.py` 逐字比對的只有**頁面名稱與分頁標籤**，
`test_architecture.py` 掃的是 import 邊界與檔案大小。**一句描述行為的散文，
沒有任何守門看得懂。**

代價不只是「少一顆按鈕」。`delete_account()` 的引用檢查涵蓋 `transaction_templates`
與 `recurring_schedules`，所以一個看不見的封存模板會讓它引用的帳戶**永遠刪不掉**，
而使用者看到的訊息是「這個帳戶已經有交易紀錄」—— 那個帳戶一筆交易也沒有。
這個死結存在了半年，沒有人回報，因為沒有人想得到要去看模板。

**結論**：把封存做成真的封存（四層都補齊，兩頁列出全部並加「狀態」欄），
並在 §5 留下一段話說明它曾經描述了一件不存在的事。

**不要再做**：

- **不要相信 `docs/architecture/*.md` 裡描述行為的句子，除非能在程式裡指出對應的函式。**
  改文件之前先 `grep` 那個動詞（`restore`、`delete`、`cancel`…）確認它真的存在。
- **寫下「A 與 B 的行為相同」時，去確認 B 有的方法 A 也有。** 這一句是整段錯誤的
  源頭 —— 它把「應該一樣」寫成了「已經一樣」。
- 新功能只做一半時，**文件寫實際做到的那一半**，剩下的寫進 `roadmap.md`。
  在架構文件裡先寫上「將來會是這樣」，讀的人分不出那是規格還是現況。

---

## 2026-08-23 灰階梯度直接拿前 N 階，兩片看起來一模一樣

**情境**：資產總覽新增資產占比圓環。專案規則是「彩色只留給金額與警示」，所以圓環走
六階灰。

**做了什麼**：`colors.CHART_SLICES[index]` —— 第 n 片拿第 n 階。

**為什麼失敗**：相鄰兩階的對比只有 1.23（那是**刻意**的，六階要鋪滿一條梯度）。
但**大部分人只有三四個帳戶**，於是實機上永遠只用得到最淺的那三階 ——
截圖上「郵局活儲」與「悠遊聯名」是兩片一樣的淺灰。分得出來的六階，
在最常見的情況下退化成分不出來的三階。

**結論**：改成在整條梯度上**平均取樣**（`slice_colors()`）：三片拿頭、中、尾，
兩片拿最淺與最深。片數少的時候對比反而最大 —— 而片數少正是使用者最想一眼比出來的時候。

**不要再做**：設計一組「N 選 1」的色階時，不要只看 N 最大的那個情況。
**先問「最常見的 N 是多少」**，那才是使用者每天看到的畫面。
純看 QSS 或色碼清單看不出這件事，一定要照 `AGENTS.md` 說的真的截一張圖。
...
七條全部回報有鑑別力，接著跑 `Verify.ps1 -Ui`，`test_account_type_is_always_cash` 失敗，
說資料庫裡出現了 `bank`。

**做了什麼**：先懷疑是測試寫錯，再懷疑 `resolve_app_paths()` 是不是繞去讀了真實資料
（那會是邊界違規，所以優先查）。都不是 —— `git diff` 對那個檔案是空的，原始碼是 `"cash"`，
但跑出來就是 `bank`。

**為什麼失敗**：`.pyc` 的有效性只看來源檔的 **mtime ＋ 檔案大小**。對照腳本把
`account_type: str = "cash"` 改成 `"bank"` —— **兩個字串一樣長**，所以還原之後檔案大小
完全相同，而還原時寫入的新 mtime 又剛好被記進了破壞期間產生的 `.pyc`。
Python 於是認為快取還新鮮，直接跑**被破壞過的 bytecode**。

`lessons.md` 2026-08-18 已經有一條「被自己的 `.pyc` 騙了」，但那條講的是**注入**那一半
（改了程式但跑到舊的，於是以為守門沒抓到）。**還原**那一半沒寫，而它更難發現 ——
因為那時候 `git diff` 是乾淨的，所有「檢查你有沒有改壞」的直覺全部失效。
...
順帶一提，這個「錯誤」抓到了一件對的事：那條守門在它自己被破壞的情況下確實會紅。
問題不在守門，在對照腳本。

**不要再做**：
- 不要以為 `git diff` 乾淨就等於跑的是那份程式碼。**`.pyc` 是第三份狀態**，
  它不在 git 裡，也不在你剛讀過的檔案裡。
- 不要挑「跟原文一樣長」的破壞字串就覺得比較安全 —— 一樣長正好是最危險的那種，
  它讓 `.pyc` 的失效判斷完全失靈。
- 對照腳本跑完之後，**再單獨跑一次目標測試確認它是綠的**。這一步只花一秒，
  而它正好會抓到還原不完全。
...
後來為了「統一」而跑了一次全域正規化，一口氣把 **54 個無關檔案**標成已修改 ——
因為**這個 repo 的工作樹本來就不是統一 CRLF**，有些檔案 checkout 出來就是 LF。
那個「統一」是我自己想像出來的規則。

**結論**：

- 任何「改一下再還原」的腳本一律走 `read_bytes()` / `write_bytes()`。
  片段裡的 `\n` 在比對前換成**那個檔案自己**的換行，不要用一個全域常數。
- **不要為了整齊去改自己沒有要改的檔案。**
- 判斷「內容到底有沒有變」用 `git hash-object -- <file>` 跟
  `git rev-parse HEAD:<file>` 比 —— `git status` 的 ` M` 在 `core.autocrlf=true`
  底下只代表位元組動過，經過 filter 之後可能一模一樣。這一次 73 個 ` M` 裡面
  只有 7 個內容真的變了。

**不要再做**：不要用文字模式讀寫「原封不動還原」的檔案（`newline=""` 只管寫入端，
擋不住讀取端的正規化）。也不要用 heredoc 裡的腳本去改**含跳脫序列的原始碼** ——
`\n` 會在 shell、heredoc、Python 字面值之間被解讀好幾次，這一輪連續寫壞三次，
每次都是把 `\n` 變成真的換行而產生語法錯誤。改檔案就用檔案編輯工具。
