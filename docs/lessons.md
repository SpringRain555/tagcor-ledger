...
## 2026-08-26 壞掉的樣式若「看起來像設計」，就沒有人會回報它

**情境**：使用者問「當前選取用白色勾選、黑色未勾選比較不直觀，你會建議改成方框內
打勾嗎？還是只是我不習慣？」
**做了什麼**：`styles.qss` 覆寫了 `QCheckBox::indicator`（為了在深色底上畫出看得見的
方框），已勾選就填成實心白。
**為什麼失敗**：**QSS 一碰 subcontrol，Fusion 就不再畫它自己的圖形** —— 跟
`QComboBox::drop-down` 那個坑完全同源，而那一條早就寫在 `AGENTS.md` 上了。
差別在**症狀的外觀**：drop-down 消失後留下一塊空白方格，一看就是壞的，當場就被發現；
勾號消失後留下的是一塊**實心白方塊**，那是個常見的現代勾選框樣式，看起來完全像
刻意設計。於是它從第一天就是壞的，而沒有人回報 —— 使用者甚至先懷疑是自己不習慣。
**結論**：方框與勾號二選一，除非給圖。實測過的死路兩條：SVG 需要 Qt SVG image
plugin（PySide6 選配，本機 conda env 沒裝），QSS 的 `url()` 不吃 `data:` URI ——
兩者失敗的樣子都是「那一格靜靜地不畫圖」，不報錯。最後打包 PNG（標準 ＋ `@2x`），
由 `tools/icons/make_check_icon.py` 產生，路徑在 `theme.py` 載入時代換進 QSS。
**不要再做**：不要因為「沒人回報」就當成沒問題 —— **會被回報的是看起來壞掉的東西，
不是壞掉的東西。** 覆寫任何 subcontrol 之後要問一次「原本那個圖形去哪了」，並且
真的截圖看一眼；`grep` 一次 `::` 開頭的 QSS 規則就能盤點出所有有這個風險的地方。

---

## 2026-08-26 `setMaximumWidth` 是上限不是保證，而固定高度的表會被捲軸吃掉一列

**情境**：使用者回報「操作設定 → 項目」的表格因為項目名稱太長而出現左右捲軸。
**做了什麼**：`fit_to_contents()` 把表格的 `maximumWidth` 設成欄寬總和（v0.19 起），
每一欄則是 `ResizeToContents`。當初認為這樣就不會有捲軸。
**為什麼失敗**：那個做法只處理了**寬度有剩**的那一半 —— 它解的是「表拉滿視窗而
表頭只畫到最後一欄，右邊留下一條接縫」。但 `setMaximumWidth` 只是上限：名稱長到
`ResizeToContents` 算出來的總和**超過**分頁真的能給的寬度時，欄位照樣溢出，
而上限對這件事一句話都沒有。第二層代價更難看出來：這些表同時是 `fit_rows` 的
**固定高度**，橫向捲軸會從那個高度裡扣掉自己的厚度，**最後一列因此被切掉** ——
畫面上看起來像資料沒載完，而不是像「有一條捲軸」。
既有的兩條掃全分頁守門（`test_shrink_wrapped_tables_are_never_clipped`、
`test_tables_are_not_clipped_on_a_brand_new_ledger`）**用的是短名稱**（「郵局活儲」
「交通」），所以它們從加進來那天起就沒有走到過這條路。
**結論**：有自由文字欄的收寬表要同時指定 `stretch_column`，讓那一欄在寬度不夠時
讓路（收寬與 stretch 在數學上接在同一點：`maximumWidth` 就是欄寬總和，空間夠時
stretch 欄分到的剛好是自己的內容寬度）。順帶關掉 `wordWrap` —— 列高釘死成單行，
Qt 預設開著的折行只會讓被壓縮的儲存格看起來像壞掉。
**不要再做**：不要把「有沒有設上限」當成「會不會溢出」。**每一條 geometry 守門都要
問一次「我的測試資料撐得破它嗎」** —— 撐不破的話那條守門保護的是它從沒到過的地方，
所以新的那條帶了陽性對照。也不要用 `setMaximumSectionSize()` 抄捷徑：它會在視窗
還有一大片空白的時候就把長名稱截掉。

---

## 2026-08-23 需要警告才填得對的欄位，是欄位的問題

**情境**：定存的「起存日」要填的是**目前存續中那一期**的起存日，而使用者手上的
郵局存單印的是最初那一期（112/11/15）。照抄建出來的是一份什麼都不會發生的定存。
**做了什麼**：加了一段當場的提醒（「你填的這一期在 2025/02/15 就到期了⋯⋯」）
再加一顆「改成 2026/02/15」的按鈕，把正確答案算給他按。
**為什麼失敗**：使用者的回應不是道謝，是一個問題 ——「我想記錄的日期就是
112/11/15，會建議直接顯示當前期間嗎？還是我直接填 114/11/15 比較好？」
那個問題指出：**那個欄位的正確值不是他知道的任何一個數字**。他手上有的是存單，
而欄位要的是一個推算結果；填對的唯一辦法是先填錯、看警告、按按鈕。而且按完之後
112/11/15 就被丟掉了 —— 那個他明說想記錄的日期，帳本裡沒有地方放。
**結論**：欄位要問**使用者一定知道的事實**，推算的部分由程式做並當場說出結果。
改成問「首次起存日」（存單上那一天）＋ 一行「目前存續中的是第 3 期：2026/02/15 –
2027/02/15」之後，警告與按鈕都不需要了 —— 它們本來就是在補欄位問錯的洞。
**不要再做**：發現使用者會填錯時，不要先想「怎麼提醒他」。先問**這個欄位問的是不是
他手上真的有的東西**。加警告是把設計問題轉嫁成使用者的注意力成本，而且每個看到那個
畫面的人都要付一次。

---

## 2026-08-23 缺的資料就讓它缺，不要為了序號好看而補空紀錄

**情境**：從第 3 期開始記錄一份已經滾過兩輪的定存，`deposit_terms` 只會有一列。
**做了什麼**：考慮過把第 1、2 期一起建出來，狀態標 `renewed`，讓期序從 1 連續。
**為什麼失敗**：那兩期的**實際利息與當時的牌告利率都不在帳本裡**，建出來的是兩筆
每個欄位都空著的紀錄。而 `deposit_terms` 的整個設計前提正好相反 —— 續存不改寫舊的
一期，就是為了讓歷次利率留得下來。補兩筆沒有利率的期，等於把那個保證變成謊。
**結論**：第一列的期序寫 **3**，中間不補。跳號本身就是資訊：「這份定存滾過兩輪，
但帳本從第 3 期才開始看」。
**不要再做**：不要為了讓序號連續、清單好看、或「資料完整」而建立內容是猜的紀錄。
**缺一段歷史**和**有一段假歷史**，後者嚴重得多，而且事後分不出來。

---

## 2026-08-23 十二種組合各有一條測試，而餵進去的資料有一種不存在

**情境**：查證「定存功能是否完整」時實測十二種組合的到期效果。
**做了什麼**：`test_every_combination_plans_the_right_postings` 用
`@parametrize` 掃過三種計息 × 四種到期方式，每一種都斷言本金轉回 100,000。
**為什麼失敗**：那個 100,000 是 fixture 的預設本金，而**零存整付的本金一開始就是 0**
（錢是每月存進去累積的，`test_installment_savings_ignores_the_principal_entirely`
自己就這樣寫著）。測試餵了一個現實中不存在的零存整付，於是
`transfers[0].amount_minor == 100_000` 照樣通過 —— 而真正的零存整付到期轉回的是
**0 元**。那個 bug 從 v0.9.0 活到 v0.23.0，十五個版本。
**結論**：parametrize 掃過所有分支只證明「每個分支都跑過」，不證明「每個分支都被
它自己的資料跑過」。共用的 fixture 預設值在某些分支下是**不可能出現的組合**時，
那幾格就是空的。
**不要再做**：不要讓 parametrize 的每一格共用同一份資料而不問「這一格的資料合理嗎」。
加分支的時候順手問一次：**這個組合在現實中長什麼樣？**

---

## 2026-08-23 「到期不能略過」不是設計，是 UNIQUE 約束的副作用

**情境**：檢視定存功能完整性，實測按下「略過」之後那一期怎麼走。
**做了什麼**：`skip()` 只把事件標成 `skipped`，不動那一期 —— 與「略過 = 我看過，
這一期不記」的語意一致。
**為什麼失敗**：`deposit_events` 有 `UNIQUE (term_id, event_type, due_date)`，而
`add_event()` 是 `INSERT OR IGNORE`。**略過掉的事件永遠不會再生出來。** 略過每月
領息只是少記一筆；略過**到期**會讓那一期停在「存續中」：不續存、不結清，之後任何
一天再產生都是 0 件，而畫面上完全看不出有什麼不對。使用者唯一的自救路徑是去
「修改所選期」改掉到期日騙過那個 UNIQUE —— 沒有人猜得到。
**結論**：一個操作對不同種類的資料有不同的**終局**時，那不是同一個操作。
擋下來，而且**訊息要講出替代作法** —— 一句「不能略過」會把使用者留在原地。
**不要再做**：不要假設「冪等的產生」加「可逆的狀態」等於「這件事可以重來」。
`INSERT OR IGNORE` 的冪等性同時也是**不可重生性**。

---

## 2026-08-23 業務規則黏在 `created_at` 上，十幾條測試同時變成空的

**情境**：實作 ADR-0012 的「建檔之前到期的事件不產生」。
**做了什麼**：第一版直接讀 `deposit_contracts.created_at` 的日期部分當下界 ——
欄位現成、不必 migration。
**為什麼失敗**：`generate_due(today=...)` 特意讓測試控制「今天」，而下界卻黏在
**真實的牆上時鐘**。整個 `test_deposits.py` 用的是 2026／2027 的假日期，於是十幾條
原本有意義的測試同時變成「什麼都沒產生也算通過」。跑起來全綠，實際上什麼都沒驗。
**結論**：`created_at` 是技術性的稽核時間戳，「這份合約什麼時候進帳本」是**業務規則**。
綁在同一欄上，規則就沒辦法被設定，也就沒辦法被測試 —— 而**一個測不到的規則就是一個
沒有人在看的規則**。加一欄（v9 `recorded_on`）比省一次 migration 值得。
**不要再做**：不要因為「欄位現成」就把一條會決定行為的規則掛到稽核用的時間戳上。
判準是「測試需不需要控制它」—— 需要，它就該是自己的欄位。

---

## 2026-08-23 我在 ADR 裡批評的缺陷，就在我沒看的那一半

**情境**：v0.23.0 移除定期收支時，ADR-0011 花了一段論證它不值得留下，
其中一條是「`confirm_occurrence()` 寫死 `occurred_at = 排定日`，而確認對話框沒有
日期欄位 —— 交易日期一定是錯的」。
**做了什麼**：據此否決了把定期收支加回來。
**為什麼失敗**：一天之後查定存，`_write_postings()` 寫死的是
`today_taipei()`，確認對話框同樣只問金額。**同一類缺陷，只是偏的方向相反** ——
而定存的到期項目**提前七天**出現，照著提示馬上確認，交易日期會比錢真的動的那天
早七天。我寫下那段批評的時候，手上就有另一個一模一樣的實例。
**結論**：拿一個缺陷去否決 A 的時候，**先在 B 身上找同一個缺陷**。
批評的形狀（「某個值由程式決定，而使用者改不到」）比它落在誰身上更值得記住。
**不要再做**：不要在寫 ADR 論證「A 不如 B」時只查 A。至少把那一條批評當成
`grep` 的關鍵字在 B 上跑一次。

---

## 2026-08-23 需求文件寫對了，實作寫短了，中間沒有東西比對

**情境**：使用者指出郵局存單上寫的是「到期**及**轉存方式」，程式寫「到期轉存方式」。
**做了什麼**：查證後發現 `REQ-0007` §列舉 **從一開始就寫著正式全文**（含
「本金（息）」與「無限次數」），是實作在 v0.9.0 縮寫了四個選項名，然後一路活到 v0.23.0。
**為什麼失敗**：`test_docs_drift.py` 逐字比對的是**頁面與分頁名稱**（正本是
`navigation.LABELS`），沒有涵蓋列舉的顯示名。而 REQ 是「當時談定的需求」，
沒有人會為了確認一個下拉選單的文字回去讀它。
**結論**：需要**逐字**相同的字串，判準是「使用者拿它跟外部的東西對照嗎」。
這四個名字的用途就是把實體單據上的打勾抄過來 —— 不逐字相同，使用者每次都要
自己重新推導一次對應關係。這件事寫進 `domain/deposits.py` 的模組說明，
放在字串旁邊而不是文件裡。
**不要再做**：不要把「照抄外部文件的字串」當成一般的 UI 文案來縮短。
縮短之前先問：**這個字串在程式外面有沒有一個正本？**
