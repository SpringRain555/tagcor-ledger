...
## 0.24.0 - 定存：照存單填就好，補記歷史不再灌爆待確認

起點是三個問題：存單上「到期及轉存方式」的用詞、那四個選項有沒有完整實作、
以及「帳戶建立的時間晚於定存真正的起始日期，會有金額跑到待確認嗎？」

第三個的答案是**會**，而且比想像中糟。查證的過程挖出另外六件事，全部在這一版一起修。

> **同一天做了兩輪。** 第一輪擋住了補記歷史的灌入（§1），並在對話框加一段警告加
> 一顆「改成 ⋯」的按鈕。使用者的追問指出那個做法在補一個更根本的問題 ——
> **欄位問錯了東西** —— 於是第二輪把欄位改成問存單上那一天（§2），警告與按鈕
> 一起拿掉。兩輪的過程都留在
> [ADR-0012](decisions/ADR-0012-deposit-events-start-at-record-date.md) 裡，
> 因為「為什麼不是第一版那樣」是這一版最值得記住的事。

### 1. 建檔之前就到期的事件不再產生

**理由與被否決的替代方案寫在
[ADR-0012](decisions/ADR-0012-deposit-events-start-at-record-date.md)。**

使用者照郵局存單抄起存日（112/11/15 存入、113/11/15 到期），而他的郵局帳戶是當天
才建的、期初餘額填的是存摺上當下的數字。2026-08-23 實測：
...
那些利息早就進了郵局帳戶，**已經含在期初餘額裡** —— 全部確認下去就是記第二次。

- **schema v9**：`deposit_contracts.recorded_on`，既有列用 `created_at` 的日期部分回填。
- `generate_due()` 的下界從此是 `contract.recorded_on`。上界仍是 7 天前置。
- **已結束的合約不再產生任何項目**（`list_active_terms()` 同時濾期與合約兩側）。

**為什麼是新的一欄而不是直接讀 `created_at`。** 讀得到，第一版也真的是這樣寫的 ——
但 `created_at` 是技術性的稽核時間戳，而下界是業務規則。綁在一起的代價立刻出現：
`generate_due(today=...)` 特意讓測試控制「今天」，下界卻黏在真實時鐘上，於是十幾條
原本有意義的測試同時變成「什麼都沒產生也算通過」。**一個測不到的規則就是一個沒有人
在看的規則。**

### 2. 填的是存單上那一天，程式算出是第幾期（schema v10）

擋下來只解決一半：那份定存會安靜地躺在清單上什麼都不做。第一版的做法是「填錯就
警告，並給一顆改日期的按鈕」——**而那是在補一個更根本的問題**：

> 那個欄位要填的是「目前存續中那一期」的起存日，而使用者手上那張紙印的是
> 112/11/15。於是欄位的正確值不是他知道的任何一個數字，得靠警告才問得出來。
> **一個需要警告才填得對的欄位，是欄位的問題。**

所以 `deposit_contracts.opened_on`（**首次起存日**）：

- 對話框問的就是紙上那個數字。
- `current_term()` 算出該建立哪一期，**同時算出期序** —— 上面那個例子建立的是
  2026-02-15 – 2027-02-15，而它是**第 3 期**。
- 那一行說明**永遠都在**，不只在填錯的時候：它回答「按下建立會發生什麼」。
- 不自動轉存的說「這份定存已經結束」（沒有下一期可以滾過去）；本息續存的多一句
  「本金請填目前存摺上的金額」—— 那個數字每期都含前一期的利息，而程式算不出來。
- 警告與那顆按鈕一起拿掉了。

**中間那幾期不建立資料列。** 它們的實際利息與當時的牌告利率都不在帳本裡，憑空生出
空紀錄就是捏造事實 —— 那正是 `deposit_terms` 一貫的立場（續存不改寫舊的一期，就是
為了讓歷次利率留得下來）。**跳號是誠實的，補號不是。**

定存有了三個日期，glossary 與 state-machines §7 各有一張表釘住它們：
`opened_on`（存單）／`recorded_on`（帳本開始追蹤，同時是產生的下界與算期序的當下）／
`DepositTerm.start_date`（這一期）。

### 3. 三處計算修正

| | 症狀 | 修法 |
|---|---|---|
| **存本取息到期利息重複算** | 每月領息已經把整年的利息發完了，到期事件卻又建議整期總額 —— 照建議值確認，帳上的利息是實際的**兩倍** | `suggest_maturity_interest_minor()`，存本取息一律回 0。`actual_interest_minor` 改記已確認領息的合計，實際年利率才反推得出來 |
| **零存整付到期本金是 0** | `principal_minor` 一開始就是 0（本金是每月存入累積的），而到期直接轉它 | `matured_principal_minor()`。使用者在「修改所選期」填過正數本金的話以他填的為準 |
| **本金沒動也記一筆轉帳** | 「利息轉入帳戶」設成定存帳戶本身時撞上 `TRANSFER_SAME_ACCOUNT` | 來源與目的相同、或金額 0 就不產生那一筆 |

那個十二種組合的 parametrize 測試沒抓到零存整付那一條，是因為它餵的
`principal="100000"` 是一個現實中不存在的零存整付。**測試資料不真，斷言就不真** ——
那個 bug 活了十五個版本。

### 4. 到期不能略過

`settle_event()` 只改事件狀態，而 `deposit_events` 有
`UNIQUE (term_id, event_type, due_date)` ＋ `INSERT OR IGNORE` —— 略過掉的到期事件
**永遠不會再生出來**，那一期就停在「存續中」：不續存、不結清，之後任何一天再產生
都是 0 件，而畫面上看不出有什麼不對（2026-08-23 實測）。

擋下來的訊息要講出替代作法：利息不想記就確認、金額填 0；整份定存不再追蹤就結束
合約或中途解約。**一句「不能略過」會把使用者留在原地。**

### 5. 交易日期改成到期日，而且問得出來

`_write_postings()` 以前寫死 `today_taipei()`。而到期項目**提前七天**出現，
照著提示馬上確認會讓交易早七天。

確認對話框從單欄金額換成小表單（入帳日期 ＋ 實際金額），日期預設事件的到期日，
銀行晚一兩天入帳時改得掉。`simple_form` 跟著多一種 `DateField`。

> 這與 [ADR-0011](decisions/ADR-0011-drop-recurring-schedules.md) 拿來否決定期收支的
> 缺陷是**同一類**（日期由程式決定、對話框沒有欄位可改），只是偏的方向相反。
> 我當時寫下那段批評，而同樣的東西就在定存這邊。

### 6. 「結束合約」與「中途解約」

`close_contract()` 從 v0.9.0 就存在於 store 裡，而 application／controller／UI
**三層都沒有接它** —— 同時 `DEPOSIT_CONTRACT_IN_USE` 的訊息與刪除確認框都寫著
「請改用『結束合約』」。使用者被指去做一件做不到的事。

`DepositTermStatus.TERMINATED`（已解約）同樣是宣告了而沒有路徑：列舉裡有、
CHECK 裡有、`primitives.py` 連中文都準備好了，`state-machines.md` §7 的狀態圖甚至
畫著那條線。**與 v0.22.0 的模板恢復是同一類**，見 [`lessons.md`](lessons.md)。

現在定存頁有三顆按鈕，界線是「動不動到錢」：

| | 動到錢嗎 | 什麼時候用 |
|---|---|---|
| **結束合約** | 不 | 已經沒有存續中的期了，只是還掛在清單上。還有存續中的期時擋下來並指向中途解約 |
| **中途解約** | 會 | 還在存續中就要提前解約。本金與利息**都由使用者填**，不試算違約息（REQ-0007 §邊界）|
| **刪除所選合約** | 不 | 記錯了。只有從未入帳過的才刪得掉 |

結束的合約預設收起來，勾「顯示已結束的合約」看得到 —— **看不見但還在**是這個專案
吃過虧的形狀，所以那個核取方塊跟按鈕一起加，不是之後再說。

**`DepositTermStatus.MATURED` 從列舉拿掉。** 沒有任何程式碼寫得出它：確認到期是
原子的，中間不存在「到期了但還沒處理」這個資料狀態。CHECK 那一側留著 —— 砍它要
重建整張表，而允許集合比實際用到的大不會造成錯誤資料。

### 7. 四個「到期及轉存方式」改回存單上的全文

`REQ-0007` §列舉 從一開始就寫著全文，而實作從 v0.9.0 起用的是縮寫版：

| 存單／REQ-0007 | v0.9.0–v0.23.0 的實作 |
|---|---|
| 到期**及**轉存方式 | 到期轉存方式 |
| 本金**（息）**自動轉存本人帳戶 | 本息自動轉存本人帳戶 |
| 本金**無限次數**自動轉期續存，利息轉存帳戶 | 本金自動轉期續存，利息轉存帳戶 |
| 本息**無限次數**自動轉期續存 | 本息自動轉期續存 |

**這一欄需要逐字相同，別欄不需要。** 使用者在那個下拉選單前面做的事就是把實體單據
上的打勾抄過來；只要文字不是逐字相同，他就得自己重新推導一次對應關係，而選項 2 與
選項 4 的差別本來就只有「本金」與「本息」一個字。

唯一沒抄進來的是選項 2 後半的「轉存成功後，本單自動失效」—— 那是郵局那張紙自己的
作廢規則，跟帳本要記什麼無關。

### 拆檔

`ui/pages/deposits.py` 原本裝著「一個分頁」加「兩張對話框」，加上新東西會超過 700 行
的煙霧偵測器。三張對話框搬到 `ui/widgets/deposit_dialog.py`，比照
`ui/widgets/template_dialog.py`。

### 測試

新增 49 條（668 → 717），其中值得一提的：

- `test_events_that_fell_due_before_the_record_date_are_not_generated` —— 這一版存在的理由。
- `test_the_floor_does_not_swallow_events_after_the_record_date` —— 陽性對照。
  沒有它的話，一個「什麼都不產生」的實作也能讓上一條通過。
- `test_the_transaction_lands_on_the_due_date_not_the_day_you_pressed_confirm` ——
  斷言比的是**事件的到期日**，不是任何一個「今天」。用今天比的話，把實作改回
  `today_taipei()` 這條照樣會過。
- `test_a_v8_database_upgrades_to_v9_and_backfills_the_record_date` —— 回填不可省：
  留空的話那份合約的下界是空字串（比任何日期都小），等於沒有下界。
- `test_recording_a_passbook_date_creates_the_term_that_is_live_today` ——
  使用者實際會做的那個動作，從存單到「第 3 期」整條走完。
- `test_the_renewed_term_keeps_counting_from_the_rolled_sequence` ——
  從第 3 期記進來的，續存之後是第 4 期不是第 2 期。

`tests/ui/conftest.py` 多一個 `make_deposit` fixture：四個檔案都要「有一件待確認」
當前提，而它們踩的坑一模一樣（`recorded_on` 要跟著起存日往前拉，否則斷言會悄悄
變成「0 件也算通過」）。

## 0.23.0 - 移除定期收支

**理由與被否決的替代方案寫在 [ADR-0011](decisions/ADR-0011-drop-recurring-schedules.md)。**
...
| | |
|---|---|
| 資料表 | `recurring_schedules`、`scheduled_occurrences`（`migrate_v8` DROP，順序不可反）|
| store | `ScheduleStore`、`OccurrenceStore`、`stores/drafts.py` |
| application | `AutomationService` → `TemplateService`（`automation.py` → `templates.py`）|
| controller | `AutomationSection` → `TemplateSection` |
| UI | 「操作設定 → 定期收支」分頁、`RecurringPage`、`DraftDialog` → `TemplateDialog` |
| domain | `RecurringSchedule`、`ScheduledOccurrence`、`dates.next_due_date()` |
| 錯誤碼 | `SCHEDULE_*`、`OCCURRENCE_*`；`AUTOMATION_*` 改名為 `TEMPLATE_*` |

**改名不是順手做的。** 移除之後 `AutomationService` 裡沒有一項是自動的，
留著那個名字只會讓下一個人以為還有背景工作在跑。同理 `DraftDialog` 的
「草稿」來自「模板與定期收支都是一份還沒生效的草稿」，而那個對照組不在了。

### 待確認留著，但瘦了一圈

定存還在餵它：到期、每月領息、零存整付的每月存入，三者都要**照存摺填實際金額**。
那個提醒與定期收支的不同 —— **銀行那一端自己會動的錢，沒有人會通知你。**
...
| 「全部確認」 | `batch_confirm_valid()` 只處理定期收支。**定存刻意不批次** |
| 「繼續產生」與漏期提示 | 「一次最多 366 期」是排程獨有的上限。定存只看未來 7 天，一次做完而且冪等 |
| `InboxEditDialog`（110 行）| 它編的是 `scheduled_occurrences`。定存只問一個實際金額 |
...
1. **定存頁補上「產生到期與領息項目」。** README 從以前就寫著這顆按鈕存在，
   而程式裡一直沒有 —— 手動觸發本來混在定期收支頁那顆裡，那一頁走了就沒有入口了。
2. **`deposit_event_values()` 的日期本來印 ISO 字串。** 它在這一版之前沒有任何頁面
   用它（待確認走的是自己的 `inbox_values()`，而那一份有轉格式），所以沒有人看到。
   兩份合併成一份的時候差一點就把畫面上的 `2021/01/15` 換成 `2021-01-15`。
3. **`delete_account()` / `delete_category()` 的引用檢查**拿掉了兩張已經不存在的表。

### 保留下來的兩條測試

- **「確認出來的交易要走共用寫入路徑」** 搬到 `test_deposits.py`。那條守的是
  「一筆交易長什麼樣只有一個地方說了算」，而定存確認同樣會建立交易。
- **「確認入帳要連動交易紀錄與側邊欄」** 留在 `test_inbox_page.py`，改走定存。

**「一次操作一個 correlation_id」沒有搬** —— 那個不變量對定存**不成立**：
一次確認可能寫出兩筆交易（本金轉帳 ＋ 利息收入），而 `transactions.correlation_id`
是 UNIQUE，所以那邊刻意每一筆各自產生一個。搬過去會是一條斷言錯誤規則的測試。

## 0.22.1 - 模板看得出自己是什麼；選取列看得出是哪一列

### 一、模板頁：兩欄「這是什麼」與一個講得出名字的標籤

- 表格從「名稱／類型／金額／備註／狀態」變成
  **「模板名稱／類型／帳戶／類別／金額／備註／狀態」** ——
  只列名稱與金額時，兩個都叫「午餐」的模板在填進記帳頁之前分不出誰是誰，
  而分辨它們正是這一頁存在的理由。
  名字由**一句查詢 join 回來**（`TemplateRow`，比照 `CategoryNode` 組合原本的
  dataclass 而不是攤平複製）；分開查就是 1+N。
- 帳戶與類別兩欄的拼法**與交易紀錄共用 `entry_target_text()`** ——
  轉帳是「郵局 → 現金」＋類別留白，收入／支出反過來。以前
  `transaction_values()` 自己拼了一份，再抄一次就會分岔。
- 對話框第一格從「名稱」改成 **「模板名稱」／「定期收支名稱」**。
  那張表單裡每一列都是某個東西的名稱（帳戶、類別、項目），寫「名稱」等於沒說。

### 二、選取列現在看得出來

底色 `SELECTED` 對一般列底 `SURFACE` 的對比只有 **1.20** —— 那是「幾乎看不出哪一列
被選中」的量化值。修法**不能只是把它調亮**：亮到 `#333340` 時支出紅對選取列的對比就
掉到 4.36，破掉「所有文字／底色組合 >= 4.5」那條守門，而選取列正是最想看清楚金額的
時候。

所以是兩件事一起：底色亮到上限 `#2E2E38`（1.34），**再加上下兩條 1px 橫線**。
橫線是形狀，不吃明度預算。

**框線會吃掉文字的高度，所以 `padding` 同步從 7px 收成 6px。** Qt 的 `::item` 是
content-box，而列高被 `defaultSectionSize(34)` 釘死 —— 直接加 2px 框線就是從中文的
可用高度裡扣。`test_selecting_a_row_does_not_squeeze_the_text` 逐項對算 7 = 6 + 1，
守的是下一個想把框線加粗的人。

### 三、「排序…」改叫「排序設定」

那個刪節號是桌面慣例（「按下去會開一個視窗」），文字也沒有被截斷 —— 實際量過是
`排`(U+6392)`序`(U+5E8F)`…`(U+2026) 三個字元、48px，按鈕遠比它寬。但使用者讀成
「文字被吃掉了」，那個慣例在這裡就沒有發揮作用。四頁一起改。

### 關於定期收支

**它不連網。** `today_taipei()` 讀的是本機系統時鐘。但它也**不是純手動** ——
每次開程式都會跑一次 `generate_due()`（`wiring.py`），另有兩顆按鈕。
產生出來的一律是待確認草稿，不會自己入帳。

「模板可以完全取代定期收支」這件事在評估中，**這一版沒有動它**。
差別只有一項：定期收支會在到期那天主動在側邊欄跳一個待確認數字。

## 0.22.0 - 資產占比圓環；模板與定期收支的「封存」終於是真的封存

### 一、資產總覽的帳戶表右邊多了一張占比圓環

表格回答「每個帳戶有多少」，圓環回答「比重長什麼樣」—— 同一份資料的兩種讀法，
所以並排而不是上下。規則全部在新的 `ui/widgets/asset_share.py`。

**灰階，沒有為它破例。** 「彩色只留給金額與警示」那條規則的代價本來就是不能用
分類色票，而占比其實是由圖例上的數字講的 —— 色階只負責讓人看出哪一片比較大。
色票是 `colors.CHART_SLICES`（六階），**刻意不加進 `ALL_TOKENS`**：那個集合是
QSS 的允許清單，而圓環是 `QPainter` 畫的。

三個真的想過的取捨：

| 情形 | 怎麼做 | 為什麼 |
|---|---|---|
| **餘額為負的帳戶** | 不畫，另外用一行字寫出名稱與金額 | 圓餅對負數沒有定義；取絕對值畫會讓一個把錢吃掉的帳戶看起來像一份資產。占比的**分母因此是正餘額合計不是總資產**，圖例底下把那個數字寫出來 |
| **超過六個帳戶** | 最小的那些併成「其他（N 個帳戶）」 | 上限等於色階數：再多一片就沒有分得出來的灰可以給它 |
| **一片都畫不出來** | 整段收起來 | 空的圓框看起來像壞掉或還沒載入，與「沒有定存合約就不顯示定存」同一條 |

計算與畫圖分開：`build_shares()` 完全不碰 Qt，所以上面那幾條是毫秒級的純函式測試，
不必用 UI 測試去量「圓環上有幾片」。

### 二、模板與定期收支的「封存」以前等於刪除

**這一版真正的理由。** `state-machines.md` §5 從 v0.7 起就寫著兩者
「轉移與帳戶／類別相同（`active ↔ archived`）」—— 而 `restore_template` /
`restore_schedule` 在 store、application、controller **三層都不存在**，
兩個分頁的 `refresh()` 也沒有帶 `include_archived`。所以按下「封存」之後，
那一列就從畫面上永遠消失了。

代價不只是「少一顆恢復按鈕」：`delete_account()` 的引用檢查涵蓋
`transaction_templates` 與 `recurring_schedules`，所以**一個看不見的封存模板會讓
它引用的帳戶與類別永遠刪不掉**，而使用者看到的訊息是「這個帳戶已經有交易紀錄」——
那個帳戶一筆交易也沒有。

現在兩頁都列出全部並帶「狀態」欄，封存與刪除是兩條分開的路：

| | 恢復 | 刪除的條件 |
|---|---|---|
| **模板** | 有，會擋使用中同名（`TEMPLATE_ACTIVE_NAME_CONFLICT`） | **永遠刪得掉** —— schema 裡沒有任何一張表指向 `transaction_templates` |
| **定期收支** | 有，不擋同名（它沒有那條唯一索引） | 從未產生過待確認項目（`SCHEDULE_IN_USE`） |

**順帶修掉一顆只有在列出封存項目之後才會踩到的地雷**：`new_template()` 把 `status`
寫死成 `"active"`，而 `save_template()` 是 UPSERT 且會寫 `status = excluded.status` ——
`DraftDialog.save()` 的 `replace()` 少帶那個欄位，「改個備註」就會把一筆封存的模板
靜悄悄變回使用中。

### 三、用詞

- 模板的 **「套用到記帳」→「填入記帳頁」**。它做的事是跳到記帳頁預填一筆，
  而「套用」聽起來像已經生效了。交易紀錄的「複製到記帳」**不動**。
- **「封存模板」→「封存／恢復所選模板」**，與帳戶／類別／項目三頁同一個說法。
  紅色（`dangerButton`）從它身上移到「刪除模板」—— 封存現在真的可逆了，
  而畫面上的紅色要對應「按下去救不回來」。

### 順帶澄清一個誤解

「刪除未使用」**不需要先封存**。`delete_account()` / `delete_category()` 完全不看
`status`，只看有沒有被引用 —— 封存與刪除是兩條平行的出路，不是先後關係。
判準表寫進 `ui-workflows.md`「操作設定」那一節。
