...
## 0.24.1 - 記帳頁照填寫順序問，設定表格的欄寬會讓路，勾選框有勾號了

實機使用之後的三件事。前兩件是「畫面沒有照使用者實際做事的樣子排」，
第三件是一個從第一天就壞著、但**因為看起來像設計而沒有人回報**的東西。

### 1. 記帳頁的欄位順序改成使用者實際填寫的順序

```text
流向 →（轉帳對象）→ 帳戶 →（轉入帳戶）→ 類別 → 項目 → 日期 → 金額 → 備註
```

金額從第 3 列移到日期後面。**它仍然是主角** —— 粗體、`Ctrl+N` 直接聚焦到它 ——
但「重要」與「先問」是兩件事：其他欄位多半沿用上次或用預設值，一路確認下來，
游標最後落在唯一每次都要動手的那一格才順。

**強調的手段跟著改了：`amountInput` 只留 `font-weight`，字級與高度與其他欄位統一。**
它以前同時放大三個維度（15pt vs 12pt、字重 600 vs Medium、40px vs 30px 高），而那是
金額還排在第三列、緊貼著「流向」的年代訂的 —— 在最上面當標題看剛好。搬到日期與備註
之間之後，上下都是一般高度的欄位，同樣的對比就從「標題」變成「中間凸一塊」。
**樣式一個字都沒動，是順序讓它換了個意思** —— 使用者實機看到就是這個感覺。

只在轉帳時出現的兩列各自貼著自己的第一層：轉帳對象是流向的第二層、
轉入帳戶是「帳戶」的另一半。

順手修掉一個既有的不一致：widget 的建立順序本來就是這個順序，而 Qt 的預設 Tab
順序跟的是建立順序 —— 所以在這之前按 Tab 走到第二格會跳到「帳戶」，而不是眼睛
看到的「金額」。現在視覺順序與 Tab 順序第一次對得起來。

新守門 `test_the_entry_form_asks_in_the_order_the_user_fills_it` 量的是 y 座標，
不是 `addRow` 的呼叫順序。

### 2. 操作設定的表格：欄寬不夠時由某一欄讓路，不再冒出橫向捲軸

**症狀**：項目名稱一長，「項目」分頁底下就多一條左右捲軸。

**根因**：`fit_to_contents()` 設的是 `setMaximumWidth`，那是**上限不是保證**。
每一欄都是 `ResizeToContents`，名稱長到欄寬總和超過分頁能給的寬度時，欄位照樣溢出。
更糟的是這些表同時是 `fit_rows` 的**固定高度**，捲軸會從那個高度裡扣掉自己的厚度 ——
**最後一列被切掉**，看起來像資料沒載完。

**做法**：收寬的表現在也可以指定 `stretch_column`（以前兩者互斥）。那一欄不是去吃
多餘的寬度，而是**寬度不夠時由它讓路**。兩件事在數學上接在同一點：`maximumWidth`
就是欄寬總和，所以空間夠的時候 stretch 欄分到的剛好是它自己的內容寬度（畫面與
改動前一模一樣），空間不夠的時候只有它被壓縮、文字省略成 `…`。

| 分頁 | 讓路的欄位 |
|---|---|
| 帳戶、類別 | 名稱（第 0 欄）|
| 項目 | 項目（第 1 欄；第 0 欄是類別名稱，那個不會長）|
| 模板 | 備註（第 5 欄）|
| 定存 | 合約名稱（第 0 欄）；期別表全是有界欄位，不需要 |

`setup_table()` 另外關掉 `wordWrap`（Qt 預設是開的）—— 列高被釘死成單行，
折行只會讓被壓縮的儲存格看起來像壞掉，而不是乾淨的省略號。

**沒有用 `setMaximumSectionSize()`**：兩行就能擋掉捲軸，但它會在視窗還有一大片空白
的時候就把長名稱截掉，而那正是使用者現在的情形（長項目名 ＋ 寬視窗）。

新守門 `test_a_long_item_name_never_makes_the_table_scroll_sideways` 帶陽性對照
（先斷言那個名字真的撐破可用寬度，否則測試等於沒作用）—— 既有那兩條掃全分頁的
守門用的是短名稱，證明不了這件事。

### 3. 勾選框終於有勾號了

**症狀**：使用者問「白色勾選、黑色未勾選比較不直觀，還是只是我不習慣？」

**根因**：不是不習慣 —— 勾號從第一天就不見了。QSS 一碰 `QCheckBox::indicator`，
Fusion 就不再畫它自己的圖形，跟 `AGENTS.md` 早就寫著的 `QComboBox::drop-down`
那條完全同源。差別在症狀的外觀：drop-down 壞掉留下一塊空白方格，一看就知道；
勾號壞掉留下的是**實心白方塊**，看起來完全像刻意設計，所以沒有人回報。

**為什麼不能乾脆不覆寫**：Fusion 的方框外框色是 `palette.window().darker(140)`
推導的，而本專案的 Window 是 `#0D0D0F` —— 外框在深色底上完全看不見，未勾選那一格
會變成一片空白。**方框與勾號二選一，除非給圖。**

**做法**：打包 PNG（`check.png` ＋ `check@2x.png`，各約 300 / 600 bytes），
由 `tools/icons/make_check_icon.py` 用 QPainter 產生 —— 兩個尺寸必須是同一個形狀
等比例放大，手畫的話 `@2x` 的線寬遲早跟標準版對不起來。路徑由 `theme.py` 的
`resolve_stylesheet()` 在載入時代換進 QSS（Qt 的 `url()` 只認檔案系統路徑，
所以樣式表沒辦法完全靜態）。取不到圖檔時整行拿掉，退回實心白 —— 留著壞掉的
`url()` 更糟，Qt 會連 `background-color` 都不畫，方框整個消失。

**兩條實測過的死路**：SVG 要 Qt SVG image plugin（PySide6 的選配元件，本機的
conda env 就沒裝，`QImageReader.supportedImageFormats()` 裡沒有 `svg`）；
QSS 的 `url()` 不吃 `data:image/png;base64,...`。兩者失敗的樣子都是「那一格靜靜地
不畫圖」，不報錯。

**這是專案第一個圖檔**，`AGENTS.md` 的「不打包圖檔」那句話跟著改了。
`pyproject.toml` 的 `package-data` 加上 `*.png`。四條新守門在
`tests/unit/test_resources.py`：兩個尺寸都在且真的有畫到東西（陽性對照）、
佔位字串會被換成真檔案路徑且是正斜線（反斜線在 `url()` 裡會靜默失敗）、
取不到圖檔時整行拿掉而方框還在、產生腳本的勾號顏色與 `colors.BG` 一致。

理由與被否決的替代方案：五個勾選框裡有兩個守著不可逆操作（`還原前先建立備份`、
`重製前先建立備份`），那兩個不該要求使用者知道「實心＝開」這個慣例。
與「選取列靠底色 ＋ 上下橫線」「金額同時有正負號與右對齊」是同一條原則 ——
**第二個線索要用形狀**。

### 4. 文件：提款也是轉帳（沒有程式改動）

`AGENTS.md` 的「對外轉帳慣例」多一小段。從郵局領現金＝錢在自己的兩個帳戶之間移動、
總資產不變，所以它就是**轉帳・我的帳戶之間**。

**不記成「郵局一筆支出 ＋ 現金一筆收入」**：總資產碰巧會對，但支出總額與收入總額
各被灌水一次，類別統計也跟著髒掉。**也不新增「提款」流向**：按下「轉帳」之後
「轉帳對象」預設就停在「我的帳戶之間」，第四顆按鈕省不到任何一次點擊，代價卻與
[ADR-0010](decisions/ADR-0010-external-transfers.md) 否決的「方案甲」一模一樣。
**不需要新的 ADR** —— 這個結論沒有推翻任何東西，它是 ADR-0010 判準的又一次套用。
