...
| **類別** | 類別／項目數／狀態。**每一個類別都有自己的一列，不管它有沒有子項目**。上方有搜尋與狀態篩選，點表頭可排序，「上移／下移」調自訂順序 |
| **項目** | 所屬類別／項目／狀態。上方有搜尋、所屬類別與狀態篩選，點表頭可排序，「上移／下移」調自訂順序（**只在同一個類別之內**）|
...
- **自訂順序用「上移／下移」，那是這兩頁的預設順序。** 常用的排前面、少用的沉底，
  而**同一份順序也用在記帳頁的類別／項目下拉** —— 名冊排好了但下拉沒跟著，等於沒排。
  - 只在**同一組**之內移動（類別跟類別、同一個類別底下的項目跟項目）。
    跨類別是「換類別」，不是「調順序」，這裡不做。
  - **點表頭改成依某一欄排序時，上移／下移會停用** —— 那時候畫面上那一列的鄰居
    不是儲存順序裡的鄰居，「上移」沒有可以解釋的結果。**再點一次表頭收回箭頭**
    就回到自訂順序（`setSortIndicatorClearable(True)` 的三段循環）。
  - 移動送的是**畫面上那個鄰居的 id**，不是「往上一格」。這一頁可以搜尋與篩選，
    兩者不一定相同 —— 送畫面上看得到的那個，結果才會跟眼睛一致。
